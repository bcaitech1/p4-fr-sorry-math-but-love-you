...
class SATRNDecoder(nn.Module):
    ...
    def forward(
        ...
        if is_train and random.random() < teacher_forcing_ratio:
            ...
            out = []
            num_steps = batch_max_length - 1 # 231
            target = torch.LongTensor(src.size(0)).fill_(self.st_id).to(device) # b - [START] token
            ...
            for t in range(num_steps):
                ...
                _out = self.generator(tgt)  # [b, 1, c] 4,1,245
...
class SATRNDecoder_soft(nn.Module):
    ...
    def forward(
        self, src, text, t, target, features, is_train=True, batch_max_length=50, teacher_forcing_ratio=1.0
    ):
...
class MySATRN_en(nn.Module):
    def __init__(self, FLAGS, train_dataset, checkpoint=None):
        super(MySATRN_en, self).__init__()
...
class MySATRN_de(nn.Module):
    def __init__(self, FLAGS, train_dataset, checkpoint=None):
        super(MySATRN_de, self).__init__()

        self.decoder = SATRNDecoder_soft(
            num_classes=len(train_dataset.id_to_token),
            src_dim=FLAGS.SATRN.decoder.src_dim,
            hidden_dim=FLAGS.SATRN.decoder.hidden_dim,
            filter_dim=FLAGS.SATRN.decoder.filter_dim,
    ...
    def forward(self, input, expected, t, target, features, is_train, teacher_forcing_ratio):
        dec_result = self.decoder(
            src=input,
            text=expected[:, :-1],
            t = t,
